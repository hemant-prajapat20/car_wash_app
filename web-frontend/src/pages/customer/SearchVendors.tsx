      ...
      {filteredVendors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVendors.map((vendor, i) => {
            const isUnavailable = vendor.availability && vendor.availability.isAvailable === false;
            ...
            return (
              <motion.div 
                ...
                className={`bg-white border border-slate-100 rounded-[2rem] p-5 shadow-sm hover:shadow-xl transition-all group flex flex-col h-full ${isUnavailable ? 'opacity-80' : 'hover:border-blue-100'}`}
              >
                {/* Header: Image & Ratings */}
                <div className="flex items-start gap-4 mb-5">
                  <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center shrink-0 border border-slate-100 overflow-hidden relative">
                    {vendor.avatar ? (
                      <img src={vendor.avatar} alt={vendor.companyName} className={`w-full h-full object-cover ${isUnavailable ? 'grayscale' : ''}`} />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isUnavailable && (
                      <div className="mt-2 inline-flex items-center gap-1 bg-rose-50 text-rose-600 border border-rose-100 px-2 py-0.5 rounded-md">
                        <span className="text-[9px] font-bold uppercase tracking-widest">
                          {vendor.availability.reason || 'Temporarily Closed'}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="pt-4 border-t border-slate-100 grid grid-cols-2 gap-3 mt-auto">
                  ...
                  <button 
                    onClick={() => !isUnavailable && navigate(`/customer/book?vendor=${vendor._id}`)}
                    disabled={isUnavailable}
                    className={`py-3 text-[12px] font-bold text-white rounded-xl shadow-lg transition-all flex items-center justify-center gap-1.5 ${
                      isUnavailable 
                  ...
                  >
                    ...
                    {!isUnavailable && <ChevronRight size={14} />}
                  </button>
                </div>
              </motion.div>
            );
